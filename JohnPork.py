...
for i in range(20):
    topic_diffs = diff_df.iloc[i]  # Select differences for the specific topic
    topic_name = diff_df.index[i]

    # Sort differences and find the closest five to zero, ensuring "COMP" is included
    sorted_diffs = topic_diffs.abs().sort_values()
    closest_topics = sorted_diffs.head(5).index.tolist()
    if 'COMP' not in closest_topics:
        closest_topics.append('COMP')
    
    # Prepare data for plotting
    plot_data = topic_diffs[closest_topics]
    colors = ['orange' if topic == 'COMP' else 'gray' for topic in closest_topics]
    
    # Plotting
    plt.figure(figsize=(8, 5))
    plot_data.plot(kind='bar', color=colors)
    plt.title(f"Difference of Topic {topic_name} Against MATH (Top 5 Closest to Zero)")
